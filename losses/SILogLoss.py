...
class SILogLoss(nn.Module):
    ...
    It does not use the implementation in the official code release for that paper.
     """
    def __init__(self, args, warm_up=True,
                 warm_iter=100):
        super().__init__()
        self.name = 'SILog'
        self.args = args
    ...
    def forward(self,
        ...
        if self.warm_up:
            if self.warm_up_counter < self.warm_iter:
                g = torch.log(depth_pred + self.eps) - torch.log(depth_gt + self.eps)
...
class SILogLossMidas(nn.Module):
    """Scale-invariant log-loss as defined in Bhat et al.'s Adabins paper.
    ...
    def forward(self,
        ...
        if interpolate:
            depth_pred = nn.functional.interpolate(depth_pred, depth_gt.shape[-2:], mode='bilinear', align_corners=True)
        #depth_pred = nn.functional.relu(depth_pred)

        if depth_mask is not None:
            depth_pred = depth_pred[depth_mask]
            depth_gt = depth_gt[depth_mask]
        
        # n_points = depth_gt.shape[-2] * depth_gt.shape[-1]
        depth_gt = (depth_gt - torch.median(depth_gt)) / ((depth_gt - torch.median(depth_gt)).abs().mean())
        depth_pred = (depth_pred - torch.median(depth_pred)) / ((depth_pred - torch.median(depth_pred)).abs().mean()+1e-6)
        loss = (depth_gt - depth_pred).abs().mean()
        return loss