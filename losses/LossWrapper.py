...
from losses.SILogLoss import SILogLoss, SILogLossMidas
from losses.BinsChamferLoss import BinsChamferLoss
from .loss_midas import TrimmedProcrustesLoss
...
class LossWrapper(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i, name in enumerate(self.args.loss.names):
            ...
            elif name == 'bins_chamfer':
                    self.logger.info("Using Bins Chamfer loss")
                    self.loss_components.append(BinsChamferLoss(self.args))
            elif name == 'silog_midas_myimp':
                    self.logger.info("Using silog_midas_myimp loss")
                    self.loss_components.append(SILogLossMidas())
            elif name == 'silog_midas':
                    self.logger.info("Using silog_midas loss")
                    self.loss_components.append(TrimmedProcrustesLoss())
    ...
    def forward(self, depth_pred, depth_gt, depth_mask, output_named_tuple):
        ...
        for i, component in enumerate(self.loss_components):
            if isinstance(component, (SILogLoss, SILogLossMidas, TrimmedProcrustesLoss)):
                loss += self.args.loss.coeffs[i] * component(depth_pred, depth_gt, depth_mask)
            else:
