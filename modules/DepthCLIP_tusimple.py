...
class DepthCLIP(nn.Module):
    ...
    def __init__(self, args):
       ...
       # rn50
        # self.transposed_conv_layer1 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=4, padding=0)

        # # 第二次上采样，4倍
        # self.transposed_conv_layer2 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=4, padding=0)

        # # 第三次上采样，4倍
        # self.transposed_conv_layer3 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=3, stride=4, padding=0)   
        

        # vit b16
        # self.transposed_conv_layer1 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=4, stride=2, padding=1)

        # # 第二次上采样，2倍
        # self.transposed_conv_layer2 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=4, stride=2, padding=1)

        # # 第三次上采样，2倍
        # self.transposed_conv_layer3 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=4, stride=2, padding=1)
        
        # self.transposed_conv_layer4 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=4, stride=2, padding=1)
        
        # nn.init.zeros_(self.transposed_conv_layer1.weight)
        # nn.init.zeros_(self.transposed_conv_layer1.bias)
        
        # nn.init.zeros_(self.transposed_conv_layer2.weight)
        # nn.init.zeros_(self.transposed_conv_layer2.bias)    
        
        # nn.init.zeros_(self.transposed_conv_layer3.weight)
        # nn.init.zeros_(self.transposed_conv_layer3.bias)
        
        
                
        # nn.init.zeros_(self.transposed_conv_layer4.weight)
        # nn.init.zeros_(self.transposed_conv_layer4.bias)
        ...
        match lang_strat.get("templates"):
            ...
            case "zwy_template":
                # self.sentence_templates = {"The depth of this {object_tkn} is {depth_tkn}."}
                self.sentence_templates = {"The {object_tkn} in the image is approximately {depth_tkn} from the viewer."}
        ...
        match lang_strat.get("depth_bin_centres"):
            case "paper":
                ...
                # self.depth_bin_centres = [1.00,15.00,30.00,45.00,60.00,75.00,90.00] 
        ...
        # self._frozen_params_module_list.append(self.transposed_conv_layer)
        # self._frozen_params_module_list.append(self.transposed_conv_layer1)
        # self._frozen_params_module_list.append(self.transposed_conv_layer2)
        # self._frozen_params_module_list.append(self.transposed_conv_layer3)
        # self._frozen_params_module_list.append(self.transposed_conv_layer4)
        
        # self._frozen_params_module_list.append(self.conv_layer)
        # self._frozen_params_module_list.append(self.clip.visual)
    ...
    # def deconv(self,x):
    #     x = self.relu(self.transposed_conv_layer1(x))
    #     x = self.relu(self.transposed_conv_layer2(x))
    #     x = self.relu(self.transposed_conv_layer3(x))
    #     x = self.relu(self.transposed_conv_layer4(x))
    ...
    def get_language_features(self, img_f_tkns=None, extra_tkns_override=None):
        ...
        return text_embedding.swapaxes(-2, -1).contiguous()    # permute is done because the old way used to stack on dim=1, needs to return C, N
        # return text_embeddings.swapaxes(-2, -1).contiguous()
    ...
    def forward(self, image):
        ...
        if self.args.basic.dataset == 'tusimple':     
            image_features, img_f = self.clip.encode_image(image,rank=True)   # rank loss 
        else:
            _, img_f = self.clip.encode_image(image,rank=False)   # B, HW, C where H and W are patch coordinates not pixel coordinates
        ...
        match self.args.depthclip.get("lang_corr_stage"):
            ...
            case "depthclip" | _:   # Default behaviour is the same as depthclip behaviour. Done for backwards compatibility.
                ...
                if self.args.basic.dataset == 'tusimple':
                    ...
                    # if rn50                     out_height = 7                     out_width = 7                      concatenated_out_width = 49                       concatenated_out_height = 7
                    concatenated_out_width = out_width*7
                    concatenated_out_height = out_height

                    depth_logits = depth_logits.permute(0, 2, 1).reshape(image.shape[0], self.bins, out_width, out_height)  # B, K, H, W 
                    depth_logits /= self.temperature

                    # Depth logit post-processing
                    depth_logits = self.post_lang_corr_stage(depth_logits)

                    depth_pred = F.softmax(depth_logits, dim=1)  # B,BINS,W,H
                    bin_tensor = torch.tensor(self.depth_bin_centres).to(depth_pred.device)
                    depth_pred = depth_pred * bin_tensor.reshape(1, self.bins).unsqueeze(-1).unsqueeze(-1)
                    depth_pred = depth_pred.sum(1, keepdim=True)
                    ...
                    concatenated_iamges = torch.zeros((1, 3, clip_width * batch_size, clip_height)).to(image.device)
                    depth_map= torch.zeros((batch_size, 1, clip_width, clip_height)).to(image.device)
                    concatenated_depth_gt = torch.zeros((1, 1, clip_width * batch_size, clip_height)).to(image.device)
                    for i in range(batch_size):
                        # depth_map[i] = self.depth_bin_centres[6-i]  不是order的时候应该用这个
                        depth_map[i] = self.depth_bin_centres[i]  # 为什么order的话用这个不对呢？使用order的话，是从近到远，这个应该没问题啊
                    # 随机选择图像的拼接顺序
                    import random
                    ...
                    for i, index in enumerate(indices):
                        start_width = i*224
                        end_width = (i+1)*224
                        # start_width_depth = i * out_width
                        # end_width_depth = (i + 1) * out_width
                        concatenated_iamges[:,:,start_width:end_width, :] = image[index]                  
                        concatenated_depth_gt[:, :, start_width:end_width, :] = depth_map[index]



                    _, concatenated_depth_pd_2 = self.clip.encode_image(concatenated_iamges,rank=False)
                    concatenated_depth_pd_2 = concatenated_depth_pd_2 / concatenated_depth_pd_2.norm(dim=-1, keepdim=True)



                    concatenated_depth_pd_2 = F.interpolate(concatenated_depth_pd_2, size=txt_f.shape[0])
                    concatenated_depth_logits = 100.0 * (concatenated_depth_pd_2 @ txt_f)  # B, HW, K # img_f and text_f have both been normalized, so just use a inner product
                    concatenated_depth_logits = concatenated_depth_logits.permute(0, 2, 1).reshape(1, self.bins, concatenated_out_width, concatenated_out_height)  # B, K, H, W 
                    concatenated_depth_logits /= self.temperature
                    
                    concatenated_depth_logits = self.post_lang_corr_stage(concatenated_depth_logits)
                    concatenated_depth_pred = F.softmax(concatenated_depth_logits, dim=1)
                    concatenated_depth_pred = concatenated_depth_pred * bin_tensor.reshape(1, self.bins).unsqueeze(-1).unsqueeze(-1)
                    concatenated_depth_pred = concatenated_depth_pred.sum(1, keepdim=True)
                    # Prediction post-processing
                    concatenated_depth_pred = self.post_depth_pred_stage(concatenated_depth_pred)
                    
                    # upsample
                    concatenated_depth_pred_inter =  nn.functional.interpolate(concatenated_depth_pred, concatenated_depth_gt.shape[-2:], mode='bilinear', align_corners=True)               
                    concatenated_depth_pred_224 = torch.zeros((1, 1, clip_width * batch_size, clip_height)).to(image.device)
                    for i in range(batch_size):
                        # concatenated_depth_pred_224[:,:,224*i:224*(i+1),:] = self.deconv(concatenated_depth_pred[:,:,out_width*i:out_width*(i+1),:])
                        concatenated_depth_pred_224[:,:,224*i:224*(i+1),:] = self.relu(self.conv_layer(self.relu(self.transposed_conv_layer(concatenated_depth_pred[:,:,out_width*i:out_width*(i+1),:]))))
                        
                    concatenated_depth_pred = concatenated_depth_pred_224+concatenated_depth_pred_inter
                    # concatenated_depth_pred = concatenated_depth_pred_inter
                    
                    depth_template = ['very distant', 'distant', 'slightly distant','neither distant nor close', 'slightly close', 'close', 'very close' ]
                    
                    
                    # rank loss
                    image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                    count = []
                    logits = (100.0 * image_features @ txt_f)
                    similarity = (100.0 * image_features @ txt_f).softmax(dim=-1)
                    for i in range(image_features.size()[0]):
                        values, indices = similarity[i].topk(5)
                        if len(indices)==0:
                            print(similarity)
                        # count.append(int(crowd_count[indices[0]]))
                        count.append(depth_template[indices[0]])
                    return self.ReturnType(depth_pred=depth_pred, depth_logits=F.softmax(depth_logits), dense_features=img_f, text_features=txt_f, concatenated_depth_pred=concatenated_depth_pred,concatenated_depth_gt=concatenated_depth_gt,rank_logits=logits)
                    ...
                    # depth_pred_conv = self.deconv(depth_pred)
                    # depth_pred_conv  = self.relu(self.conv_layer(self.relu(self.transposed_conv_layer(depth_pred))))
