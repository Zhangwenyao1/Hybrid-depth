...
class DepthCLIP(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.args.depthclip.get("post_img_feat_stage") is None or self.args.depthclip.get("post_img_feat_stage") == "depthclip":
                self.post_img_feat_stage = nn.Identity()
        elif self.args.depthclip.get("post_img_feat_stage") == "interpolate_bilinear_half":
                self.post_img_feat_stage = ImgFInterpolateWrapper(scale_factor=0.5)
        elif self.args.depthclip.get("post_img_feat_stage") == "interpolate_bilinear_full":
                self.post_img_feat_stage = ImgFInterpolateWrapper(scale_factor=1)
        elif self.args.depthclip.get("post_img_feat_stage") == "BasicDecoder-4f":
        ...
        elif self.args.depthclip.get("post_img_feat_stage") == "BasicDecoder-4t":
        ...
        if self.args.depthclip.get("post_lang_corr_stage") is None or self.args.depthclip.get("post_lang_corr_stage") == 'depthclip':
                self.post_lang_corr_stage = nn.Identity()
        elif self.args.depthclip.get("post_lang_corr_stage") == "upsample-2x":
                self.post_lang_corr_stage = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False)
        elif self.args.depthclip.get("post_lang_corr_stage") == "upsample-4x":
                self.post_lang_corr_stage = nn.Upsample(scale_factor=4, mode="bilinear", align_corners=False)
        elif self.args.depthclip.get("post_lang_corr_stage") == "BasicDecoder-4f":
        ...
        elif self.args.depthclip.get("post_lang_corr_stage") == "BasicDecoder-5f":
        ...
        if self.args.depthclip.get("post_depth_pred_stage") is None or self.args.depthclip.get("post_depth_pred_stage") == 'depthclip':
                self.post_depth_pred_stage = nn.Identity()
        elif self.args.depthclip.get("post_depth_pred_stage") == "BasicDecoder-4f":
    ...
    def forward(self, image):
        ...
        if self.args.basic.dataset == 'tusimple':     
            image_features, img_f = self.clip.encode_image(image,rank=True,csa=False)
        else:
            _, img_f = self.clip.encode_image(image, csa=True)   # B, HW, C where H and W are patch coordinates not pixel coordinates
        ...
        if "imgf" in self.args.depthclip.lang_strat.get("object_tokens"):
            ...
            if self.args.depthclip.lang_strat.imgf_transformation == 'interpolate':   # not using a .get() because imgf_transformation must exist.
        ...
        elif self.args.depthclip.get("post_img_feat_stage") == 'BasicDecoder-4f' or self.args.depthclip.get("post_img_feat_stage") == 'BasicDecoder-4t':
        ...
        if self.args.depthclip.get("lang_corr_stage") == "per-imgf-depthclip":
        ...
        elif self.args.depthclip.get("lang_corr_stage") == "direct-out":
                sys.exit("ERROR: DIRECT OUTPUT (txt_f to depth directly, txt_f shape BxHWxK) NOT IMPLEMENTED YET")
                
        else:   # Default behaviour is the same as depthclip behaviour. Done for backwards compatibility.
                ...
                if self.args.basic.dataset == 'tusimple':
                    ...
                    for i in range(batch_size):
                        ...
                        depth_map[i] = self.depth_bin_centres[6-i]  # 为什么order的话用这个不对呢？使用order的话，是从近到远，这个应该没问题啊
                    # 随机选择图像的拼接顺序
                    import random
                    ...
                    _, concatenated_depth_pd_2 = self.clip.encode_image(concatenated_iamges,csa=True)
                    ...
                    bin_tensor = torch.tensor(self.depth_bin_centres).to(concatenated_depth_pred.device)
