...
class DepthCLIP(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.scalor = torch.tensor(27.0, requires_grad=True)
        ...
        # self._encoder_params_module_list.append(self.clip.visual)
        self._encoder_params_module_list.append(self.clip.visual)
        self._encoder_params_module_list.append(self.scalor)
        self._encoder_params_module_list.append(self.decoder)
    ...
    def forward(self, image):
        ...
        if self.args.basic.dataset == 'tusimple':     
            image_features, input_features = self.clip.encode_image(image, rank=True, pose=False)
        else:
            _, input_features = self.clip.encode_image(image,pose=False)   # B, HW, C where H and W are patch coordinates not pixel coordinates
        ...
        if self.args.basic.dataset == 'tusimple':    
            ...
            for f in input_features:
                ...
                depth_logits = depth_logits.permute(0, 2, 1).reshape(f.shape[0], 7, h, w).contiguous()
            ...
            for i in range(4, -1, -1):
                ...
                x = [upsample(x)]
                if self.use_skips and i > 0:
                    if x[0].shape[-1]==input_features[i - 1].shape[-1]:
                ...
                if i in self.scales:
                    depth_pred = self.scalor/self.sigmoid(self.convs[("dispconv", i)](x))
            ...
            _, concatenated_iamges_features = self.clip.encode_image(concatenated_iamges, pose=False)


                
            concat_align_feature = []
            for f in concatenated_iamges_features:
                h, w = f.shape[2], f.shape[3]
                f = f.reshape(f.shape[0], f.shape[1], -1).permute(0, 2, 1)
                f = f/f.norm(dim=-1, keepdim=True)
                img_f = F.interpolate(f, size=txt_f.shape[0])
                depth_logits = 100.0 * (img_f @ txt_f)
                depth_logits = depth_logits.permute(0, 2, 1).reshape(f.shape[0], 7, h, w).contiguous()
                concat_align_feature.append(depth_logits)
                
                
            x = concatenated_iamges_features[-1]
            for i in range(4, -1, -1):
                x = self.convs[("upconv", i, 0)](x)
                x = [upsample(x)]
                if self.use_skips and i > 0:
                    if x[0].shape[-1]==concatenated_iamges_features[i - 1].shape[-1]:
                        x += [concatenated_iamges_features[i - 1]]
                    else: 
                        x += [upsample(concatenated_iamges_features[i - 1])]
                x = torch.cat(x, 1)
                x = self.convs[("upconv", i, 1)](x)
                if i in self.scales:
                    concatenated_depth_pred = self.scalor/self.sigmoid(self.convs[("dispconv", i)](x))
                ...
                # rank loss
                # image_features = image_features.reshape(image_features.shape[0], image_features.shape[1], -1).permute(0, 2, 1)
                # image_features = F.interpolate(image_features, size=txt_f.shape[0])
            ...
            return self.ReturnType(depth_pred=depth_pred, depth_logits=F.softmax(align_feature[-1]), dense_features=img_f, text_features=txt_f, concatenated_depth_pred=concatenated_depth_pred,concatenated_depth_gt=concatenated_depth_gt,rank_logits=logits)
