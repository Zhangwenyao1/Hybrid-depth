...
#         ToTensor_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(
#                 mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
#             )])
#         image_reordered = self.transform_convert(image[0].cpu(), ToTensor_transform) 
#         # image_reordered = np.transpose(image_reordered, (1, 2, 0))
#         # 保存图像
#         image_pil = Image.fromarray(np.uint8(image_reordered))
#         image_pil.save("./image.jpg")
        
#         image_reordered = np.transpose(depth_gt[0].cpu(), (1, 2, 0))



#         image_pil = np.uint8(image_reordered)
#         # 保存图像
#         # image_pil = Image.fromarray(np.uint8(image_reordered))
        
#         image_2d = np.squeeze(image_pil)

# # 创建灰度图像对象
#         image_pil = Image.fromarray(image_2d, mode='L')
#         image_pil.save("./depth.jpg")
        
#         exit(0)
        
        # image_pil = Image.fromarray(np.uint8(image[0].cpu()))
        # image_pil.save("./image.jpg")

        # # 保存深度图
        # depth_gt_pil = Image.fromarray(np.uint16(depth_gt[0].cpu()))
        # depth_gt_pil.save("./depth_gt.png")
       
        
        output_named_tuple = self.model(image)
        if self.args.basic.dataset == 'tusimple':
            depth_pred = output_named_tuple.concatenated_depth_pred
            depth_gt = output_named_tuple.concatenated_depth_gt
            depth_mask = (depth_gt > self.args[self.args.basic.dataset].min_depth) & (depth_gt <= self.args[self.args.basic.dataset].max_depth)
            depth_loss = self.loss(depth_pred=depth_pred, depth_gt=depth_gt, depth_mask=depth_mask, output_named_tuple=output_named_tuple)
            losses = self.compute_losses(output_named_tuple.rank_logits, 'train')
            rank_loss = losses['rank_loss']
            # real_loss = 
            loss = depth_loss + rank_loss
            ...
            self.log("train/depth loss", depth_loss, on_step=True) 
            self.log("train/rank loss", rank_loss,on_step=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.args.basic.dataset == 'tusimple':
            ...
            depth_mask = (depth_gt > self.args[self.args.basic.dataset].min_depth) & (depth_gt <= self.args[self.args.basic.dataset].max_depth)
            depth_loss = self.loss(depth_pred=depth_pred, depth_gt=depth_gt, depth_mask=depth_mask, output_named_tuple=output_named_tuple)
            losses = self.compute_losses(logits, 'val')
            rank_loss = losses['rank_loss']
            loss = depth_loss + rank_loss
    ...
    def train_dataloader(self):
        if self.args.basic.dataset == 'tusimple':
            train_loader = RegressionDataModule(self.args, mode='train').data
        elif self.args.basic.get("use_adabins_dataloader") == True:
    ...
    def val_dataloader(self):
        if self.args.basic.dataset == 'tusimple':
            val_loader = RegressionDataModule(self.args,mode='online_eval').data
        elif self.args.basic.get("use_adabins_dataloader") == True:
    ...
    def predict_dataloader(self):
        if self.args.basic.dataset == 'tusimple':
            predict_loader = RegressionDataModule(self.args,mode='online_eval').data
        elif self.args.basic.get("use_adabins_dataloader") == True:
