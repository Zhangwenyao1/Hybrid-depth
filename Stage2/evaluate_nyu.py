...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        dataset = datasets.NYUTestDataset(opt.data_path, filenames,
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                _, feature, depth, _, _ = encoder(input_color, pose=True)
    ...
    results_edit=open('mpvitxsmall_results.txt',mode='a')
    results_edit.write("\n " + 'dataset: %s '%(opt.dataset))
    results_edit.write("\n " + 'model_name: %s '%(opt.load_weights_folder))
    results_edit.write("\n " + 'resolutions: {} x {}'.format(encoder_dict['width'], encoder_dict['height']))
    if not opt.disable_median_scaling:
        results_edit.write("\n " + ("{:>8} | " * 3).format("med", "std", "mean"))
        results_edit.write("\n " + ("&{: 8.3f}  " * 3).format(med, np.std(ratios / med), np.mean(ratios)))
