...
class DepthCLIP(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.transposed_conv_layer = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=32, stride=32, padding=0).cuda()
        ...
        match lang_strat.get("object_tokens"):
            ...
            case "zwy_obj":
                self.object_tokens = ["car","pedestrian","cyclist","van","truck","tram","misc"]
    ...
    def forward(self, image):
        ...
        if self.args.basic.dataset == 'tusimple':     
            ...
            image_features, img_f = self.clip.encode_image(image,rank=False)   # B, HW, C where H and W are patch coordinates not pixel coordinates
        ...
        match self.args.depthclip.get("lang_corr_stage"):
            ...
            case "depthclip" | _:   # Default behaviour is the same as depthclip behaviour. Done for backwards compatibility.
                ...
                if self.args.basic.dataset == 'tusimple':
                    ...
                    concatenated_iamges = torch.zeros((1, 3, 224 * batch_size, 224)).to(image.device)
                    depth_map= torch.ones((batch_size, 1, out_width, out_height)).to(image.device)
                    concatenated_depth_gt = torch.zeros((1, 1, out_width * batch_size, out_height)).to(image.device)
                    for i in range(batch_size):
                        depth_map[i] = self.depth_bin_centres[6-i]  
                    # 随机选择图像的拼接顺序
                    import random
                    ...
                    for i, index in enumerate(indices):
                        ...
                        concatenated_depth_gt[:, :, start_width_depth:end_width_depth, :] = depth_map[index]
                    ...
                    concatenated_out_width = 49
                    concatenated_out_height = 7
                    ...
                    concatenated_depth_pred_224 = torch.zeros((1, 1, 224 * batch_size, 224)).to(image.device)
                    for i in range(batch_size):
                        concatenated_depth_pred_224[:,:,224*i:224*(i+1),:] = self.conv_layer(self.relu(self.transposed_conv_layer(concatenated_depth_pred[:,:,7*i:7*(i+1),:])))
                    ...
                    # import cv2
                    # image_array = depth_pred.cpu().numpy()
