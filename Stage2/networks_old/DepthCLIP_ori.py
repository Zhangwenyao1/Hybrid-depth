...
class DepthCLIP(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = OmegaConf.load(args.config_file)
        self.args = self.args
        args = self.args
        ...
        # self.pose_projection = nn.Linear(512,3)
        # self.axisangle = torch.empty(1, 768)
        # self.translation = torch.empty(1,768)
        ...
        self.lenth = 16
        # self.proj_axisangle = nn.Parameter(torch.randn(240, 3))
        # self.proj_translation = nn.Parameter(torch.randn(240, 3))
        
        # nn.init.normal_(self.axisangle, std=0.02)
        # self.axisangle = nn.Parameter(self.axisangle)
        # self.scalor = 1.00
        # self.scalor = nn.Parameter(self.scalor)
        # self.scalor = torch.tensor(0.0, requires_grad=True)
        # self.scalor = nn.Parameter(self.scalor)
        # nn.init.normal_(self.translation, std=0.02)
        ...
        match lang_strat.get("depth_bin_centres"):
            ...
            case "dset-even-256":
                ...
                # Evenly-spaced across the whole range of the dataset being used.
                dset_range = args[args.basic.dataset].max_depth - args[args.basic.dataset].min_depth
                bin_width = dset_range / self.lenth
                self.depth_bin_centres = [args[args.basic.dataset].min_depth + (i * (bin_width)) + (bin_width / 2) for i in range(self.lenth)]
                self.bins = len(self.depth_tokens)
            case _:
        ...
        match axisangle_strat.get("axisangle_templates"):
            # Static templates (i.e. the template words are static, but the depth/object tokens can be either.)
            case "paper":
                self.axisangle_sentence_templates = ["This two {photo_tkn} is {axisangle_tkn_tkn}"]
            case "paper-v1":
                self.axisangle_sentence_templates = ["The transformation for this two {photo_tkn} appears to {axisangle_tkn}"]
        ...
        match axisangle_strat.get("photo_tokens"):
            # Static object tokens
            case "paper":
        ...
        match axisangle_strat.get("axisangle_tokens"):
            # Static depth tokens
            case "axisangle":
        ...
        match axisangle_strat.get("axisangle_bin_centres"):
            case "axisangle-even-7":
                # Evenly-spaced across the whole range of the dataset being used.
                axisangle_dset_range =  1
                axisangle_bin_width = axisangle_dset_range / 7
                self.axisangle_bin_centres = [0 + (i * (axisangle_bin_width)) + (axisangle_bin_width / 2) for i in range(7)]
                self.axisangle_bins = len(self.axisangle_tokens)
            case "axisangle-even-128":
                # Evenly-spaced across the whole range of the dataset being used.
                axisangle_dset_range =  1
                axisangle_bin_width = axisangle_dset_range / self.lenth
                self.axisangle_bin_centres = [0 + (i * (axisangle_bin_width)) + (axisangle_bin_width / 2) for i in range(self.lenth)]
                self.axisangle_bins = len(self.axisangle_tokens)
            case _:
                sys.exit("Error: language strategy depth bin centres set not recognised")
        ...
        translation_strat = self.args.depthclip.translation_strat
        self.translation_extra_tkns_cfg = {}    # Contains k:v pairs of token name, number of extra tokens with that name.
        self.translation_extra_tkns_learnables = {}   # Contains refs to extra LEARNABLE tokens, if any.  
        self.translation_extra_tkns_lookup = {}   # Key = new token, value = learnable parameter for that token.
        self.translation_extra_tkns_reverse_lookup = {}   # Key = token index, value = learnable parameter for that token.
        
        match translation_strat.get("translation_templates"):
            # Static templates (i.e. the template words are static, but the depth/object tokens can be either.)
            case "paper":
                self.translation_sentence_templates = ["This two {photo_tkn} is {translation_tkn}"]
            case "paper-v1":
                self.translation_sentence_templates = ["The transformation for this two {photo_tkn} appears to {translation_tkn}"]
            
            # Learnable templates. <x>o<y>d means x learnable tokens, then the object token, then y more learnable tokens, then the depth token.
            case "learned-static-4o4d":
                self.translation_sentence_templates = ["<|prompt_0|> <|prompt_1|> <|prompt_2|> <|prompt_3|> {photo_tkn} <|prompt_4|> <|prompt_5|> <|prompt_6|> <|prompt_7|> <|prompt_8|> <|prompt_9|> <|prompt_10|> <|prompt_11|> <|prompt_12|> <|prompt_13|> <|prompt_14|> <|prompt_15|> {translation_tkn}."]
                # self.translation_sentence_templates = ["<|prompt_0|> <|prompt_1|> <|prompt_2|> <|prompt_3|> {photo_tkn} <|prompt_4|> <|prompt_5|> <|prompt_6|> <|prompt_7|> {translation_tkn}."]
                self.translation_extra_tkns_cfg["prompt"] = 16
                self.translation_extra_tkns_learnables_prompt = nn.ParameterList(values=[torch.normal(mean=0, std=0.02, size=(1, 512)) for i in range(16)])
                self._extra_learnable_params_list += self.translation_extra_tkns_learnables_prompt
                for i in range(16):
                    self.translation_extra_tkns_lookup[f"<|prompt_{i}|>"] = self.translation_extra_tkns_learnables_prompt[i]
            case _:
                sys.exit("Error: sentence templates set not recognised.")        
        
        

        match translation_strat.get("photo_tokens"):
            # Static object tokens
            case "paper":
                # self.photo_tokens = ["photoes"]
                self.axisangle_object_tokens = ["photoes"]
            case "learned-static-1":
                # 1 learned object token. NOT input-dependent: represented as learnable parameters.
                self.translation_object_tokens = [f"<|object_{i}|>" for i in range(1)]
                self.translation_extra_tkns_cfg["object"] = 1
                self.translation_extra_tkns_learnables_object = nn.ParameterList(values=[torch.normal(mean=0, std=0.02, size=(1, 512))])
                self._extra_learnable_params_list += self.translation_extra_tkns_learnables_object
                for i in range(1):
                    self.translation_extra_tkns_lookup[f"<|object_{i}|>"] = self.translation_extra_tkns_learnables_object[i]
            case _:
                sys.exit("Error: language strategy photoes tokens set not recognised")
            
            
        match translation_strat.get("translation_tokens"):
            # Static depth tokens
            case "translatation":
            ...
            case "learned-static-7":
                # 7 learned depth tokens. NOT input-dependent: represented as learnable parameters.
                self.translation_tokens = [f"<|translation_{i}|>" for i in range(7)]
                self.translation_extra_tkns_cfg["translation"] = 7
                self.translation_extra_tkns_learnables_depth = nn.ParameterList(values=[torch.normal(mean=0, std=0.02, size=(1, 512)) for i in range(7)])
                self._extra_learnable_params_list += self.translation_extra_tkns_learnables_depth
                for i in range(7):
                    self.translation_extra_tkns_lookup[f"<|translation_{i}|>"] = self.translation_extra_tkns_learnables_depth[i]
            
            case "learned-static-128":
                # 256 learned depth tokens. NOT input-dependent: represented as learnable parameters.
                self.translation_tokens = [f"<|depth_{i}|>" for i in range(self.lenth)]
                self.translation_extra_tkns_cfg["depth"] = self.lenth
                self.translation_extra_tkns_learnables_depth = nn.ParameterList(values=[torch.normal(mean=0, std=0.02, size=(1, 512)) for i in range(self.lenth)])
                self._extra_learnable_params_list += self.translation_extra_tkns_learnables_depth
                for i in range(self.lenth):
                    self.translation_extra_tkns_lookup[f"<|depth_{i}|>"] = self.translation_extra_tkns_learnables_depth[i]        
                    
            # Learned depth tokens


        match translation_strat.get("translation_bin_centers"):
            case "translation-even-7":
                # Evenly-spaced across the whole range of the dataset being used.
                translation_dset_range = 1
                translation_bin_width = translation_dset_range / 7
                self.translation_bin_centres = [0 + (i * (translation_bin_width)) + (translation_bin_width / 2) for i in range(7)]
                self.translation_bins = len(self.translation_tokens)
            case "translation-even-128":
                # Evenly-spaced across the whole range of the dataset being used.
                translation_dset_range = 1
                translation_bin_width = translation_dset_range / self.lenth
                self.translation_bin_centres = [0 + (i * (translation_bin_width)) + (translation_bin_width / 2) for i in range(self.lenth)]
                self.translation_bins = len(self.translation_tokens)
            case _:
                sys.exit("Error: language strategy depth bin centres set not recognised")
                    
        self.translation_texts = []
        for template in self.translation_sentence_templates:
            for obj in self.translation_object_tokens:
                for translation in self.translation_tokens:
                    self.translation_texts.append(template.format(photo_tkn=obj, translation_tkn=translation))        
        self.translation_texts_tokenized, self.translation_tokenizer = clip.tokenize(self.translation_texts, extra_tkns_cfg=self.translation_extra_tkns_cfg)  # tokenize
    ...
    def get_translation_language_features(self, img_f_tkns=None, extra_tkns_override=None):
        """Get static language (no learned embeddings.
        Uses self.sentence_templates, self.depth_tokens and self.object_tokens to create language embeddings
        for each depth class.
        Follows function zeroshot_classifier() in DepthCLIP/monoclip.py, from original DepthCLIP paper's code.
        ...
        self.translation_texts_tokenized = self.translation_texts_tokenized.cuda()

        # If there are extra tokens to be used, and if the index-to-param lookup hasn't been initialised, then do that
        # Doing this here is necessary because self.device isn't set properly during __init__().
        # Overwrite img_f token if needed
        if self.translation_extra_tkns_lookup != {} and (self.translation_extra_tkns_reverse_lookup == {}):
            for k, v in self.translation_tokenizer.extra_tkns_reverse_lookup.items():
                word_tkn = self.translation_tokenizer.extra_tkns_reverse_lookup[k]
                if "imgf" in word_tkn:
                    self.translation_extra_tkns_reverse_lookup[k] = word_tkn, None
                else:
                    self.translation_extra_tkns_reverse_lookup[k] = (word_tkn, self.translation_extra_tkns_lookup[word_tkn].cuda())

        if extra_tkns_override is not None:
            for i, (k, v) in enumerate(self.translation_extra_tkns_reverse_lookup.items()):
                self.translation_extra_tkns_reverse_lookup[k] = extra_tkns_override[:, i]   # Each is Bx512 now. encode_text should handle this.

        text_embeddings = self.clip.encode_text(self.translation_texts_tokenized, self.translation_extra_tkns_reverse_lookup, self.translation_tokenizer.extra_tkns_fwd_lookup, img_f_tkns)  # embed with text encoder
    ...
    def forward(self, image, pose):
        ...
        device = image.device
        # _, img_f, axisangle, translation = self.clip.encode_image(image, self.axisangle.to(device), self.translation.to(device), pose=True)   # B, HW, C where H and W are patch coordinates not pixel coordinates
        img_f, features, axisangle, translation = self.clip.encode_image(image, pose)
        ...
        translation_txt_f = self.get_translation_language_features(img_f_tkns, extra_tkns_override)
        ...
        if axisangle is not None:
            axisangle = axisangle/axisangle.norm(dim=1, keepdim=True)
            translation = translation/translation.norm(dim=1, keepdim=True)
            axisangle_f = axisangle
            translation_f = translation
            # axisangle_f = F.interpolate(axisangle, size=axisangle_txt_f.shape[0])
            # translation_f = F.interpolate(translation, size=translation_txt_f.shape[0])
        else:
            axisangle_f = None
            translation_f = None
        ...
        pose = {}
        pose['axisangle_f'] = axisangle_f
        pose['axisangle_txt_f'] = axisangle_txt_f
        
        pose['translation_f'] = translation_f
        pose['translation_txt_f'] = translation_txt_f
        
        pose['axisangle_bin_centres'] = self.axisangle_bin_centres
        pose['translation_bin_centres'] = self.translation_bin_centres
        
        pose['fcaxisangle'] = self.fcaxisangle
        pose['fctranslation'] = self.fctranslation
        
        # pose['conv_axisangle'] = self.conv_axisangle
        # pose['conv_translation'] = self.conv_translation
        
        # pose['axisangle_linear'] = self.axisangle_linear
        # pose['translation_linear'] = self.translation_linear
        
        pose['relu'] = self.relu
        pose['lenth'] = self.lenth
        # pose['proj_axisangle'] = self.proj_axisangle
        # pose['proj_translation'] = self.proj_translation
        return outputs, features, axisangle_f, pose
