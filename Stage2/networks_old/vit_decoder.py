...
class depthclipdecoder(nn.Module):
    ...
    def forward(self, input_features, depth):
        ...
        depth_token = input_features[-1]
        depth_token = depth_token.reshape(depth_token.shape[0], depth_token.shape[1], -1).permute(0, 2, 1)
        img_f = depth_token
        ...
        depth_logits = depth_logits.permute(0, 2, 1).reshape(depth_token.shape[0], self.lenth, 6, 20).contiguous()  # B, K, H, W 
        ...
        depth_pred = depth_pred * bin_tensor.reshape(1, self.lenth).unsqueeze(-1).unsqueeze(-1)
        ...
        depth_pred = nn.functional.interpolate(depth_pred, [192, 640], mode='bilinear', align_corners=True)   
        
        outputs = {}
        # outputs[("disp", 0)] = 1/depth_pred
        x = input_features[-1]
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                outputs[("disp", i)] = 0.5*self.sigmoid(self.convs[("dispconv", i)](x)) + 0.5*1/depth_pred
                # outputs[("disp", i)] = self.sigmoid(self.convs[("dispconv", i)](x))
                
        # original 
        # + F.interpolate(depth_pred, [x.shape[2], x.shape[3]], mode="bilinear")
        ...
        # return outputs, axisangle, translation", 0)] = nn.Conv2d(16, 256, 3, 1, 1)
