...
class depthclippose(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.pose_convs[('squeeze')] = nn.Conv2d(512, 1024, 1)
        # nn.init.ones_(self.pose_convs[('squeeze')].weight)
        # nn.init.zeros_(self.pose_convs[('squeeze')].bias)
        ...
        # self.pose_text_residuals = nn.Parameter(torch.zeros([16, 6, 20]))
    ...
    def forward(self, input_features, axisangle_dict):
        ...
        # pose_enhanced_residual = axisangle_txt_f + self.alpha * self.pose_text_residuals
        axisangle = axisangle.reshape(axisangle.shape[0],axisangle.shape[1],-1).permute(0,2,1)
        axisangle_logits = 100*(axisangle @ axisangle_txt_f)
        ...
        return axisangle, translation, tsne_pose 
