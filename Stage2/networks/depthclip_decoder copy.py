...
class depthclipdecoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.scales = [0]
        self.num_ch_enc = np.array([64, 256, 512, 1024, 16])
    ...
    def forward(self, input_features, depth):
        ...
        depth_pred_inter = nn.functional.interpolate(depth_pred, [192, 640], mode='bilinear', align_corners=True)   
        # print((1/depth_pred).max())
        outputs = {}
        # outputs[("disp", 0)] = 1/depth_pred
        # x = input_features[-1]
        
        # x = self.scalor/depth_pred
        for i in range(4, -1, -1):
