...
def evaluate(opt):
    ...
    pose_encoder_path = os.path.join(opt.load_weights_folder, "encoder.pth")
    pose_decoder_path = os.path.join(opt.load_weights_folder, "pose.pth")

    # pose_encoder = networks.ResnetEncoder(opt.num_layers, False, 2)
    pose_encoder = networks.depthclipencoder(opt)
    encoder_dict = torch.load(pose_encoder_path)
    pose_encoder.load_state_dict({k: v for k, v in encoder_dict.items() if k in pose_encoder.state_dict()})

    # pose_decoder = networks.PoseDecoder(pose_encoder.num_ch_enc, 1, 2)
    # pose_decoder.load_state_dict(torch.load(pose_decoder_path))
    pose_decoder = networks.depthclippose(opt)
    pose_decoder.load_state_dict(torch.load(pose_decoder_path))
    ...
    with torch.no_grad():
        for inputs in dataloader:
            ...
            _, all_features, depth, axisangle_dict, translation_dict = pose_encoder(all_color_aug, pose=True)
            ...
            axisangle, translation = pose_decoder(pose_inputs, axisangle_dict, translation_dict)
