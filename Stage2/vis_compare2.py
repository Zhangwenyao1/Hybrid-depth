...
os.makedirs('vis_all_2', exist_ok=True)
ms = 20
for i, file in enumerate(files):
    ...
    img_all = 255 + np.zeros((224, 672*4 + 3*ms, 3), dtype=np.uint8)
    img_all[:, :672] = np.asarray(color)
    img_all[:, 672+ms:672*2+ms] = np.asarray(monodepth_img)
    img_all[:, 672*2+ms*2:672*3+ms*2] = np.asarray(manydepth_img)
    img_all[:, 672*3+ms*3:672*4+ms*3] = np.asarray(monodepth_our_img)
    Image.fromarray(img_all).save(f'vis_all_2/{i}_disp.png')
