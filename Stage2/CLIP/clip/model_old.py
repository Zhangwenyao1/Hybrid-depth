...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, pose=False):
        super().__init__()
        ...
        # the 3-layer stem
        if pose:
            self.conv1 = nn.Conv2d(6, width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        else:
...
def build_model(state_dict: dict, pose=False):
    ...
    model = CLIP(
        ...
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, pose
    )
    ...
    if pose ==True:
        if 'visual.conv1.weight' in state_dict:
            state_dict.pop('visual.conv1.weight')
