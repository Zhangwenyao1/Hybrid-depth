...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, rank=False,return_feat=False):
        def stem(x):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rank=False,return_feat=False):
...
#         x = x + self.positional_embedding.to(x.dtype)
        ...
        n = 4 
        output, total_block_len = [], len(self.transformer.resblocks)
        blocks_to_take = range(total_block_len - 4, total_block_len) if isinstance(n, int) else n
        # x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND
        
        
        # take the last four layer like depthanything
        n = 4 
        output, total_block_len = [], len(self.transformer.resblocks)
        ...
        # depth_x = self.ln_post(x[:, 1:, :])

        # print(self.logit_scale.exp())
        outputs = [self.ln_post(out) for out in output]
        token_x = [out[:, 0] for out in outputs][-1]
        depth_x = [out[:, 1:] for out in outputs]

        if self.proj is not None:
            token_x = token_x @ self.proj
            # depth_x = depth_x @ self.proj
            depth_x = [depth_x[0], depth_x[1], depth_x[2], depth_x[3]@ self.proj]
        # x = x.permute(0, 2, 1).unsqueeze(-1)
        # x = x.view(x.shape[0], x.shape[1], out_height, out_width)
        # x = x.contiguous()

        return token_x, depth_x    # B, C, H, W (where H and W are patch indices, not pixel coords)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image,rank=False,csa=False, return_middle=False):
        return self.visual(image.type(self.dtype),rank, return_feat=return_middle)
