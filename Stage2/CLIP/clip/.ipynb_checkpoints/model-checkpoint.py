...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_emb = F.interpolate(self.positional_embedding.permute(1, 0).unsqueeze(0), size=[x.shape[0]], mode='linear').reshape(-1, x.shape[0]).permute(1, 0)
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        self.layer_pose3 = self._make_layer(width * 4, layers[2], stride=2) 
        self.layer_pose4 = self._make_layer(width * 8, layers[3], stride=2) 
        ...
        # self.attnpool_axisangle = AttentionPool2d(input_resolution // 32, embed_dim, heads, output_dim)
        # self.attnpool_translation = AttentionPool2d(input_resolution // 32, embed_dim, heads, output_dim)
       
        # self.fcaxisangle_1 = nn.Linear(output_dim, 3, bias=True)
        # self.fctranslation_1 = nn.Linear(output_dim, 3, bias=True)
        # self.fcaxisangle_2 = nn.Linear(3, 3, bias=True)
        # self.fctranslation_2 = nn.Linear(3, 3, bias=True)
        # scale = width ** -0.5
        # self.proj_axisangle = nn.Parameter(torch.randn(120, 3))
        # self.proj_translation = nn.Parameter(torch.randn(120, 3))
        # self.attnpool_pose =  AttentionPool2d(input_resolution // 32, embed_dim, heads, output_dim)
        # self.fcpose_1 = nn.Linear(output_dim,6,bias=True)
        # self.fcpose_2 = nn.Linear(6, 6, bias=True)
        self.relu = nn.ReLU()
    ...
    def forward(self, x, pose, rank=False,return_feat=True):
        def stem(x):
        ...
        x = self.layer1(x) #  B, 64, 48, 160
        if return_feat:
            self.features.append(x)  # B, 256, 48, 160
        x = self.layer2(x)
        pose_x = x
        if return_feat:
            self.features.append(x)   # B, 512, 24, 80

        x = self.layer3(x)
        if return_feat:
            self.features.append(x)   #  B,1024,12,40
        x = self.layer4(x)
        if return_feat:
            self.features.append(x)  # B, 2048, 6, 20     
         

        pose_x = self.layer_pose3(pose_x)
        pose_x = self.layer_pose4(pose_x)
        ...
        if rank is False:
            depth_x = x.reshape(x.shape[0], x.shape[1], -1)
              # B HW  C
            if pose:
                # x = x.reshape(x.shape[0],x.shape[1] , 7, 7)
                # pose = self.attnpool_pose(x)
                # pose = 0.01*self.fcpose_2(self.relu(self.fcpose_1(pose)))
                # axisangle_token = pose[:,:3]
                # translation_token = pose[:,3:]
                
                # axisangle_token = self.attnpool_axisangle(x)
                # translation_token = self.attnpool_translation(x)
                # axisangle_token = 0.01 * self.fcaxisangle_2(self.relu(self.fcaxisangle_1(axisangle_token)))             
                # translation_token = 0.01 * self.fctranslation_2(self.relu(self.fctranslation_1(translation_token)))
                pose = pose_x.reshape(pose_x.shape[0], pose_x.shape[1], -1)
                axisangle_token = pose
                translation_token = pose
                # axisangle_token = axisangle_token @  self.proj_axisangle
                # translation_token = translation_token @ self.proj_translation
                axisangle_token = axisangle_token.permute(0, 2, 1)
                translation_token = translation_token.permute(0, 2, 1)
                # translation_token = translation_token.unsqueeze(1).unsqueeze(1)
            else:  
                axisangle_token = None
                translation_token = None
            depth_x = depth_x.permute(0, 2, 1)
        else:
            depth_x = x.reshape(x.shape[0], x.shape[1], -1)
            depth_x = depth_x.permute(0, 2, 1)
            x = self.attnpool(x)
        ...
        return depth_x , self.features, axisangle_token, translation_token, 
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # self.proj2 = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor, axisangle, translation, pose, rank=False,return_feat=False):
        ...
        if pose:
            axisangle_token = axisangle.expand(x.shape[0], -1, -1)
            translation_token = translation.expand(x.shape[0],-1,-1)
            x_cls = x[:,:1,:]
            x_patch = x[:,1:,:]
            x = torch.cat([x_cls, axisangle_token, translation_token, x_patch], dim=1)        
        ...
        x = self.transformer(x)              # output is 197,7,768
        x = x.permute(1, 0, 2)  # LND -> NLD # 7,197,768
        # x = self.ln_post(x)
        ...
        token_x = self.ln_post(x[:, 0, :])    
        # x = self.ln_post(x[:, , :])  # B, HW, C
        axisangle_token = self.ln_post(x[:, 1, :])
        translation_token = self.ln_post(x[:, 2, :])  
        depth_x = self.ln_post(x[:, 3:, :])

        if self.proj is not None:
            token_x = token_x @ self.proj
            depth_x = depth_x @ self.proj
        ...
        return token_x, depth_x, axisangle_token, translation_token    # B, C, H, W (where H and W are patch indices, not pixel coords)
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        if isinstance(self.visual, ModifiedResNet):
            ...
            # if  self.visual.attnpool_axisangle is not None:
            #     std = self.visual.attnpool_axisangle.c_proj.in_features ** -0.5
            #     nn.init.normal_(self.visual.attnpool_axisangle.q_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_axisangle.k_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_axisangle.v_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_axisangle.c_proj.weight, std=std)
            #     std = self.visual.attnpool_translation.c_proj.in_features ** -0.5
            #     nn.init.normal_(self.visual.attnpool_translation.q_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_translation.k_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_translation.v_proj.weight, std=std)
            #     nn.init.normal_(self.visual.attnpool_translation.c_proj.weight, std=std)
    ...
    # def encode_image(self, image,  axisangle, translation, pose, rank=False):
    def encode_image(self, image, pose, rank=False, ):
        return self.visual(image.type(self.dtype),pose)
        # return self.visual(image.type(self.dtype), axisangle, translation, pose, rank)
