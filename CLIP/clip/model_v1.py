...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        # self.layer_pose1 = copy.deepcopy(self.layer1)
        # self.layer_pose2 = copy.deepcopy(self.layer2)
        # self.layer_pose3 = copy.deepcopy(self.layer3)
        # self.layer_pose4 = copy.deepcopy(self.layer4)
        # self.axisangle_layer = nn.Conv2d(in_channels=2048, out_channels=1024, kernel_size=3, stride=1, padding=1)
        # self.translation_layer = nn.Conv2d(in_channels=2048, out_channels=1024, kernel_size=3, stride=1, padding=1)

        num_ch_enc = [256,]

        embed_dim = width * 32  # the ResNet feature dimension
        # self.convs_pose = nn.Conv2d(2048, 512, 1)
        
        
        # self.convs[("pose", 0)] = nn.Conv2d(2048, 256, 3, 1, 1)
    ...
    def forward(self, x, pose, rank=False,return_feat=True):
        ...
        # x = self.layer4(x)
        if return_feat:
            self.features.append(x)   # B, 64, 48, 160
        pose_x = x
        x = self.layer1(x) 
        if return_feat:
        ...
        if rank is False:
            ...
            # pose_x = pose_x.reshape(pose_x.shape[0], pose_x.shape[1], -1)
              # B HW  C
            if pose:
                # axisangle = self.axisangle_layer(x)
                # translation = self.translation_layer(x)
                # axisangle_token = axisangle.reshape(axisangle.shape[0], axisangle.shape[1], -1)
                # translation_token = translation.reshape(translation.shape[0], translation.shape[1], -1)

                # axisangle_token = axisangle_token.permute(0, 2, 1)
                # translation_token = translation_token.permute(0, 2, 1)
                
                # common
                # axisangle_token = depth_x.permute(0, 2, 1)
                # translation_token = depth_x.permute(0, 2, 1)
                
                # 仿照share
                # pose_token = self.relu(self.convs_pose(x))
                # pose_token = pose_token.reshape(pose_token.shape[0], pose_token.shape[1], -1)
                # axisangle_token = pose_x
                # translation_token = pose_x
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.axisangle_vector = torch.empty(3, 768)
        self.translation_vector = torch.empty(3,768)
        self.axisangle_token = nn.Parameter(self.axisangle_vector)
        self.translation_token = nn.Parameter(self.translation_vector)
    ...
    def forward(self, x: torch.Tensor):
        ...
        axisangle_token = axisangle_token.expand(x.shape[0], -1, -1)
        translation_token = translation_token.expand(x.shape[0],-1,-1)
        x_cls = x[:,:1,:]
        x_patch = x[:,1:,:]
        ...
        axisangle_token = self.ln_post(x[:, 1:4, :])
        translation_token = self.ln_post(x[:, 4:7, :])  
        depth_x = self.ln_post(x[:, 7:, :])
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, pose, rank=False, csa=False):
        return self.visual(image.type(self.dtype), pose, rank)
