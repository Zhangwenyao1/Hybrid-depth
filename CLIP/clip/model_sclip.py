...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, rank=False,return_feat=False,csa=False):
        def stem(x):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rank=False, csa=True):
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image,rank=False,csa=False):
        return self.visual(image.type(self.dtype),rank,csa=csa)
