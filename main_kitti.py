...
def main(args):
    ...
    pretrained_weights = torch.load('/code/depth_estimation/monodepth2/results/mono_model_shared_winit_wdis/models/weights_13/depthclip.pth')
    modified_weights = {}
...
# #     # 创建一个新的状态字典，其中键已正确重命名
    # model_state_dict = {
    #     k: v for k, v in modified_weights.items()
    # }
...
# #     # 加载新的状态字典到模型中
    ...
    "pytorch-lightning_version": pl.__version__  # 添加版本信息
}
    torch.save(checkpoint, './modified.ckpt')
...
#     # exit(0)
#     # # # 加载修改后的权重到新模型
#     # torch.save(pretrained_weights, './modified_weights.ckpt')
#     # pretrained_weights = torch.load('./modified_weights.ckpt')
#     # print(pretrained_weights['state_dict'].keys())
#     # model.load_state_dict(modified_weights)
    args.basic.from_checkpoint = './modified.ckpt'
    ...
    # state_dict = torch.load(args.basic.from_checkpoint)
    ...
    if args.get("validate"):
        ...
        trainer = pl.Trainer(
            limit_train_batches=7 if args.debug else None,
        ...
        args.basic.val_checkpoint = args.basic.from_checkpoint
        ...
        # tmp_ckpt = {re.sub(r"\.embeddings\.weight", "", k) if re.compile(".*extra_tkns_learnables_.*\.embeddings\.weight").match(k) else k: v for k, v in tmp_ckpt.items()}
        # tmp_ckpt.pop("data_augmentation.random_planckian_jitter.pl", None)
        ...
        predictions = trainer.predict(
            model=model,
            # ckpt_path=args.basic.val_checkpoint,
            # ckpt_path='./modified_weights.ckpt'
            # ckpt_path=None
        )
        ...
        callbacks_list.append(pl.callbacks.ModelCheckpoint(monitor="metrics/abs_rel", save_last=True, save_top_k=20, mode="min"))
        ...
        trainer = pl.Trainer(
            ...
            gradient_clip_val=gradient_clip_val,
            accelerator="auto",
            strategy=pl.strategies.ddp.DDPStrategy(find_unused_parameters=False, static_graph=False),
            # strategy=pl.strategies.ddp.DDPStrategy(find_unused_parameters=False, static_graph=True),
