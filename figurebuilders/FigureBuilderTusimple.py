...
class FigureBuilderTusimple():
    ...
    def setup_axes(self):
        ...
        self.ax[self.curr_sample, 1].set_title('Patches')
        self.ax[self.curr_sample, 2].set_title('Sim Map')
    ...
    def build(self, batch):
        ...
        samples_to_plot = min(batch['image'].shape[0]-len(batch['depth_gt']), self.num_samples)
        # samples_to_plot = 0
        images_tusimple = [batch['image'][i] for i in range(len(batch['kitti_mask'])) if batch['kitti_mask'][i]==False]
        resize_ratios_tusimple = [batch['resize_ratios'][i] for i in range(len(batch['kitti_mask'])) if batch['kitti_mask'][i]==False]
        for self.curr_sample in range(0, samples_to_plot):
            ...
            unnorm_img = np.transpose(self.unnormalize(images_tusimple[self.curr_sample]).detach().cpu().numpy(), (1, 2, 0))
            self.ax[self.curr_sample, 0].imshow(unnorm_img)
            self.ax[self.curr_sample, 0].axis('off')

            # draw sampled patches
            sampled_patches_all_lanes = batch['sampled_patches'][self.curr_sample]
            unnorm_img = cv2.cvtColor((unnorm_img*255).astype(np.uint8), cv2.COLOR_RGB2BGR)
            for i in range(len(sampled_patches_all_lanes)):
                for j in range(len(sampled_patches_all_lanes[i])):
                    ratio = resize_ratios_tusimple[self.curr_sample]
                    cv2.rectangle(unnorm_img, (int((sampled_patches_all_lanes[i][j][0]-16)*ratio), int((sampled_patches_all_lanes[i][j][1]-16)*ratio)), (int((sampled_patches_all_lanes[i][j][0]+16)*ratio), int((sampled_patches_all_lanes[i][j][1]+16)*ratio)), color=(0, 0, 255), thickness=2)
            self.ax[self.curr_sample, 1].imshow(cv2.cvtColor(unnorm_img, cv2.COLOR_BGR2RGB))
            self.ax[self.curr_sample, 1].axis('off')

            # similarity map
            similarity_map = batch['similarity_map'][self.curr_sample][0]
            self.ax[self.curr_sample, 2].imshow(similarity_map.detach().cpu().numpy())
