...
class FigureBuilder():
    ...
    def build(self, batch):
        ...
        samples_to_plot = min(len(batch['depth_gt']), self.num_samples)
        # samples_to_plot = 0
        images_kitti = [batch['image'][i] for i in range(len(batch['kitti_mask'])) if batch['kitti_mask'][i]==True]
        for self.curr_sample in range(0, samples_to_plot):
            ...
            self.ax[self.curr_sample, 0].imshow(np.transpose(self.unnormalize(images_kitti[self.curr_sample]).detach().cpu().numpy(), (1, 2, 0)))
            ...
            vmin = torch.min(batch['depth_gt'][self.curr_sample])
            vmax = torch.max(batch['depth_gt'][self.curr_sample])
            self.ax[self.curr_sample, 1].imshow(batch['depth_gt'][self.curr_sample].detach().cpu().numpy(), vmin=vmin, vmax=vmax, cmap=self.cmap1)
            self.ax[self.curr_sample, 1].axis('off')

            # Predicted Depth
            # Uses vmin and vmax from batch['depth_gt'] so they're comparable
            depth_pred_interp = F.interpolate(batch['depth_pred'][self.curr_sample][None, None, :, :], size=batch['depth_gt'][self.curr_sample].shape, mode='bilinear', align_corners=True).squeeze()
            vmin = torch.min(depth_pred_interp)
            vmax = torch.max(depth_pred_interp)
            self.ax[self.curr_sample, 2].imshow(depth_pred_interp.detach().cpu().numpy(), vmin=vmin, vmax=vmax, cmap=self.cmap2)
