...
from modules.MainRunnerLM_vit2 import MainRunnerLM
...
def main(args):
    ...
    if args.get("validate"):
        ...
        #callbacks_list.append(pl.callbacks.ModelCheckpoint(dirpath=args.paths.run_dir, every_n_epochs=1, filename="epoch_{epoch:02d}", save_top_k=-1, verbose=True))
        callbacks_list.append(CustomModelCheckpoint(dirpath=args.paths.run_dir, every_n_epochs=2, filename="{epoch}", save_top_k=-1, verbose=True))
        ...
        trainer = pl.Trainer(
            ...
            default_root_dir=args.paths.run_dir,
            logger=WandbLogger(args.basic.name, save_dir=args.paths.run_dir, project='CFMDE') if not args.debug else tb_writer
        )
