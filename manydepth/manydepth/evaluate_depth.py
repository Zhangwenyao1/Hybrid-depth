...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if not opt.use_cfmde:
            ...
            if opt.eval_teacher:
                encoder = networks.dino_clip_encoder_teacher.depthdinoclipencoder_teacher(opt)
            else:
                encoder = networks.dino_clip_encoder.DINOCLIPEncoderMatching(
                    opt,
                    input_height=opt.height, input_width=opt.width,
                    adaptive_bins=True, min_depth_bin=0.1, max_depth_bin=20.0,
                    depth_binning=opt.depth_binning, num_depth_bins=opt.num_depth_bins)
            ...
            dpt_in_channels = 0
            if 'clip' in opt.backbone_model:
                dpt_in_channels += 256
            if 'dino' in opt.backbone_model:
                dpt_in_channels += 768
            ...
            if opt.use_resnet_decoder:
                depth_decoder = networks.DepthDecoder(
                [64, 256, 512, 1024, 2048], opt.scales)
            else:
                depth_decoder = networks.depthclip_decoder.depthclipdecoder(opt, in_channels=dpt_in_channels, out_channels=[256, 512, 1024, 1024])
            ...
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if opt.eval_teacher:
                    ...
                    if isinstance(output, dict):
                        output = depth_decoder(output['features'], {'depth_text_embeddings':output['depth_text_embeddings']})
                    else:
                        output = depth_decoder(output)
                ...
                if opt.vis_dir is not None:
                    ...
                    name_dest_im = os.path.join(opt.vis_dir, "{}_disp.jpeg".format(i))
