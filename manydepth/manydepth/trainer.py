...
import networks.depthclip_decoder
import networks.dino_clip_encoder
import networks.dino_clip_encoder_teacher
import networks.dpt
...
class Trainer:
    def __init__(self, options):
        ...
        if not self.opt.use_cfmde:
            ...
            self.models["encoder"] = networks.dino_clip_encoder.DINOCLIPEncoderMatching(
                self.opt,
                input_height=self.opt.height, input_width=self.opt.width,
                adaptive_bins=True, min_depth_bin=0.1, max_depth_bin=20.0,
                depth_binning=self.opt.depth_binning, num_depth_bins=self.opt.num_depth_bins)
            ...
            dpt_in_channels = 0
            if 'clip' in self.opt.backbone_model:
                dpt_in_channels += 256
            if 'dino' in self.opt.backbone_model:
                dpt_in_channels += 768
            ...
            if self.opt.use_resnet_decoder:
                self.models["depth"] = networks.DepthDecoder(
                [64, 256, 512, 1024, 2048], self.opt.scales)
            else:
                self.models["depth"] = networks.depthclip_decoder.depthclipdecoder(self.opt, in_channels=dpt_in_channels, out_channels=[256, 512, 1024, 1024])
        ...
        #----------------------- teacher ---------------------#
        if self.opt.use_dpt_teacher:
            self.models['mono_encoder'] = networks.dino_clip_encoder_teacher.depthdinoclipencoder_teacher(self.opt)
            dpt_in_channels = 0
            if 'clip' in self.opt.backbone_model:
                dpt_in_channels += 256
            if 'dino' in self.opt.backbone_model:
            ...
            if self.opt.use_depth_text_align:
                if self.opt.cat_depth_text_logic:
                    dpt_in_channels += self.models['mono_encoder'].lenth
                else:
                    dpt_in_channels = self.models['mono_encoder'].lenth
            self.models['mono_depth'] = networks.depthclip_decoder.depthclipdecoder(self.opt, in_channels=dpt_in_channels, out_channels=[256, 512, 1024, 1024])
            ...
            if pretrained_path != "":
                ...
                state_dict = torch.load(pretrained_path)['state_dict']
                new_state_dict = {key.replace('model.', ''): value for key, value in state_dict.items()}  
                self.models['mono_encoder'].load_state_dict(new_state_dict, strict=False)

            for n,p in self.models['mono_encoder'].named_parameters():
                if 'dino' in n:
                    p.requires_grad_(False)
                else:
                    p.requires_grad_(True)
        ...
        fpath = os.path.join("/code/CFMDE-main/manydepth/splits", self.opt.split, "{}_files.txt")
        ...
        if not self.opt.debug:
            self.writers = {}
            self.writers['train'] = wandb.init(project='cfmde-manydepth', name=self.opt.model_name)
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.num_epochs):
            if self.epoch == self.opt.freeze_teacher_epoch and not self.opt.only_train_teacher:
                self.freeze_teacher()
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            clip_grad_norm_(self.models['encoder'].parameters(), 10)
            clip_grad_norm_(self.models['depth'].parameters(), 10)
    ...
    def process_batch(self, inputs, is_train=False):
        ...
        if self.train_teacher_and_pose:
            ...
            if isinstance(feats, dict):
                mono_outputs.update(self.models['mono_depth'](feats['features'], {'depth_text_embeddings':feats['depth_text_embeddings']}))
            else:
        ...
        if self.opt.only_train_teacher:
        ...
        if isinstance(features, dict):
            depth_text_embeddings = features['depth_text_embeddings']
            features = features['features']
        if depth_text_embeddings is not None:
            outputs.update(self.models["depth"](features, {'depth_text_embeddings': depth_text_embeddings}))
        else:
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for l, v in losses.items():
            ...
            wandb.log({f"{mode}/{l}":v}, step=self.step)

        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            s = 0  # log only max scale
            for frame_id in self.opt.frame_ids:
                Img = wandb.Image(inputs[("color", frame_id, s)][j].data.permute(1, 2, 0).cpu().numpy())
                wandb.log({"color_{}_{}/{}".format(frame_id, s, j): Img}, step=self.step)
                # writer.add_image(
                #     "color_{}_{}/{}".format(frame_id, s, j),
                #     inputs[("color", frame_id, s)][j].data, self.step)
                if s == 0 and frame_id != 0:
                    Img = wandb.Image(outputs[("color", frame_id, s)][j].data.permute(1, 2, 0).cpu().numpy())
                    wandb.log({"color_pred_{}_{}/{}".format(frame_id, s, j): Img}, step=self.step)
                    # writer.add_image(
                    #     "color_pred_{}_{}/{}".format(frame_id, s, j),
                    #     outputs[("color", frame_id, s)][j].data, self.step)

            disp = colormap(outputs[("disp", s)][j, 0])
            Img = wandb.Image(disp.transpose(1, 2, 0))
            wandb.log({"disp_multi_{}/{}".format(s, j): Img}, step=self.step)
            # writer.add_image(
            #     "disp_multi_{}/{}".format(s, j),
            #     disp, self.step)

            if not self.opt.only_train_teacher:
                disp = colormap(outputs[('mono_disp', s)][j, 0])
                Img = wandb.Image(disp.transpose(1, 2, 0))
                wandb.log({"disp_mono/{}".format(j): Img}, step=self.step)
                # writer.add_image(
                #     "disp_mono/{}".format(j),
                #     disp, self.step)
            ...
            if outputs.get("lowest_cost") is not None:
                ...
                Img = wandb.Image(lowest_cost.transpose(1, 2, 0))
                wandb.log({ "lowest_cost/{}".format(j): Img}, step=self.step)
                Img = wandb.Image((lowest_cost * consistency_mask).transpose(1, 2, 0))
                wandb.log({"lowest_cost_masked/{}".format(j): Img}, step=self.step)
                Img = wandb.Image(consistency_mask[0])
                wandb.log({"consistency_mask/{}".format(j): Img}, step=self.step)
                # writer.add_image(
                ...
                Img = wandb.Image(consistency_target.squeeze().transpose(1, 2, 0))
                wandb.log({"consistency_target/{}".format(j): Img}, step=self.step)
                # writer.add_image(
        ...
        for n in self.opt.models_to_load:
            ...
            if n == 'encoder':
                ...
                if min_depth_bin is not None:
                    ...
                    if isinstance(self.models['encoder'], networks.dino_clip_encoder.DINOCLIPEncoderMatching):
                        self.models['encoder'].compute_depth_bins(min_depth_bin, max_depth_bin, 0)
                    else:
